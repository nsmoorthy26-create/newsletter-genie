...
function SectionBlock({ s, theme, design, iconStyle }: SectionProps) {
  ...
  if (s.layout === "hero") {
    return (
      <header
        className={`relative overflow-hidden ${design === "compact" ? "rounded-xl p-7" : "rounded-3xl p-10 md:p-14"} ${theme.card}`}
        style={{ backgroundImage: theme.pattern }}
      >
        <Kicker s={s} theme={theme} iconStyle={iconStyle} />
        <h2 className={`mt-4 text-4xl md:text-5xl font-bold leading-tight ${theme.text}`}>
          {s.title}
        </h2>
        ...
        {s.items.length > 0 && (
          <ul className="mt-8 grid sm:grid-cols-2 gap-3">
            {s.items.map((it, i) => (
              <li
                key={i}
                className={`flex items-start gap-3 rounded-xl p-3 ${theme.text}`}
          ...
          <div className="flex flex-col items-center">
            {s.stat ? (
              <>
                ...
                <div className={`mt-12 text-xs uppercase tracking-wider ${theme.muted}`}>
                  {s.stat.label}
                </div>
              </>
            ) : (
      ...
      <section className={`${frame} ${depth} ${theme.card} ${theme.text}`}>
        ...
        <ul className="mt-5 space-y-2">
          {s.items.map((it, i) => (
            ...
            >
              <span
                className="mt-0.5 grid place-items-center size-6 rounded-full text-white text-xs font-bold"
                style={{ background: theme.accent }}
              >
                ✓
              </span>
  ...
  if (s.layout === "contact") {
    return (
      <section
        className={`rounded-3xl p-8 ${theme.text}`}
...
export function NewsletterRenderer({
  ...
  return (
    <div className={`min-h-full rounded-3xl p-6 md:p-10 ${theme.bg}`}>
      <div
        className={`mx-auto max-w-4xl ${design === "compact" ? "space-y-3" : design === "cards" ? "grid gap-5 md:grid-cols-2" : "space-y-6"}`}
      >
        <div
          className={`flex items-center justify-between text-xs uppercase tracking-[0.25em] ${theme.muted}`}
        >
          <span>{data.issue}</span>
          <span>{data.subtitle}</span>
        </div>
        <h1
          className={`text-3xl md:text-4xl font-black tracking-tight ${theme.text} ${design === "cards" ? "md:col-span-2" : ""}`}
        >
          {data.title}
        </h1>
        {data.sections.map((s, index) => (
          <div
            key={s.id}
            className={
              design === "cards" && (index === 0 || s.layout === "contact") ? "md:col-span-2" : ""
            }
          >
            <SectionBlock s={s} theme={theme} design={design} iconStyle={iconStyle} />
          </div>
        ))}
        <footer
          className={`text-center text-xs ${theme.muted} pt-4 ${design === "cards" ? "md:col-span-2" : ""}`}
        >
          {data.footer}
        </footer>
      </div>
